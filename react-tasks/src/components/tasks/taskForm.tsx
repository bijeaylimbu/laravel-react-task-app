...
export default function TaskForm({ onSubmit, initial }: { onSubmit: (p:any) => Promise<void>, initial?: any }) {
  ...
  return (
    <form onSubmit={submit} className="mb-4 border p-3 rounded">
      <input className="w-full p-2 mb-2 border rounded" value={title} onChange={e=>setTitle(e.target.value)} placeholder="Title" required />
      <textarea className="w-full p-2 mb-2 border rounded" value={description} onChange={e=>setDescription(e.target.value)} placeholder="Description" />
